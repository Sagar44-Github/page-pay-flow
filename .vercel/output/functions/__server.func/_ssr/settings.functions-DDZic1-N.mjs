import { a as useSession$1, n as createServerFn, r as TSS_SERVER_FUNCTION } from "./server-C6P-fwpQ.mjs";
import { createHash, timingSafeEqual } from "node:crypto";
//#region node_modules/.nitro/vite/services/ssr/assets/settings.functions-DDZic1-N.js
...
function sessionConfig() {
	return {
		password: process.env["SESSION_SECRET"] || "pagepay-default-secure-session-secret-key-32-chars-minimum",
		name: "pagepay-admin",
		maxAge: 28800,
		cookie: {
			httpOnly: true,
...
async function snapshot() {
	const { getConfig, overriddenKeys } = await import("./config.server-BtVvDl_U.mjs").then((n) => n.n).then((n) => n.t);
...
var adminSaveSettings = createServerFn({ method: "POST" }).inputValidator((data) => data).handler(adminSaveSettings_createServerFn_handler, async ({ data }) => {
	...
	const { updateConfig, validatePatch } = await import("./config.server-BtVvDl_U.mjs").then((n) => n.n).then((n) => n.t);
	const { resetResourceServer } = await import("./routeConfig.server-BLRas4Cf.mjs").then((n) => n.c).then((n) => n.s);
	const patch = {
...
var adminResetSettings = createServerFn({ method: "POST" }).handler(adminResetSettings_createServerFn_handler, async () => {
	...
	const { resetConfig } = await import("./config.server-BtVvDl_U.mjs").then((n) => n.n).then((n) => n.t);
	const { resetResourceServer } = await import("./routeConfig.server-BLRas4Cf.mjs").then((n) => n.c).then((n) => n.s);
	resetConfig();
	resetResourceServer();
	return {
