import{t as e}from"./jsx-runtime-CfuWmoMz.js";import{n as t}from"./MarketingPage-IkApkD27.js";import{r as n,t as r}from"./DocsLayout-BYbcoOvr.js";var i=[{id:`network`,title:`Network configuration`,body:`PagePay operates exclusively on **Algorand Testnet**. Mainnet is not supported in this reference deployment.
...
**Wallet network mismatch** is the #1 integration bug: if Pera is on Mainnet but the API quotes Testnet, signing will fail or produce invalid signatures. Always verify the network badge in Pera settings before connecting.`},{id:`usdc-asset`,title:`USDC testnet asset`,body:`Payments settle in **USDC**, not ALGO. ALGO is only required for transaction fees and minimum balance requirements.
...
**Common mistake:** funding only ALGO via the Algorand dispenser. The x402 exact payment is an **asset transfer**, not an ALGO payment.`},{id:`transaction-structure`,title:`Transaction group structure`,body:`x402 exact-AVM payments with a hosted facilitator use a **2-transaction atomic group**:
...
Both transactions must succeed atomically — if either fails, neither is committed. This protects merchants from partial payments and payers from charged-but-unfulfilled requests when combined with server-side verify-before-fulfill logic.`,code:{title:`Pre-flight checklist`,code:`# Before your first payment:
...
5. Same address connected in header as signing wallet`}},{id:`pera-signing`,title:`Pera Wallet signing (ARC-0001)`,body:`PagePay uses \`@perawallet/connect\` for browser wallet integration.
...
Omitting \`signers\` while passing a global address causes Pera to skip signing — the UI appears stuck at "Signing in Pera".`},{id:`facilitator`,title:`GoPlausible facilitator`,body:`PagePay delegates on-chain submission to the hosted **GoPlausible** facilitator at \`https://facilitator.goplausible.xyz\`.
...
**504 timeouts:** testnet congestion or facilitator load can cause slow settlement. PagePay returns retryable errors; the client should not assume payment failed until verify confirms no submission.`},{id:`merchant-address`,title:`Merchant address (payTo)`,body:"The `payTo` field in payment requirements is the Algorand address receiving USDC. In PagePay this maps to the `RESOURCE_PAY_TO` server environment variable.\n\n**Self-payment is valid:** payer and payTo can be the same address (you pay yourself in a demo). The protocol cares about exact amount settlement, not address inequality.\n\n**Verification:** after payment, confirm USDC balance increased at `payTo` via explorer or `GET /v2/accounts/{address}/assets`."},{id:`proof`,title:`Proof of payment`,body:`Never trust UI state alone. PagePay surfaces multiple independent proof layers:
...
- Receiver matches \`payTo\``},{id:`troubleshooting`,title:`Algorand troubleshooting`,body:`| Symptom | Likely cause | Fix |
| --- | --- | --- |
...
| Amount off by 10× | Decimals confusion | USDC uses 6 decimals; $0.01 = 10000 units |`}],a=e();function o(){return(0,a.jsx)(t,{children:(0,a.jsx)(r,{title:`Algorand settlement`,description:`Deep dive into Testnet configuration, USDC ASA 10458941, transaction groups, Pera ARC-0001 signing, GoPlausible facilitator, and on-chain verification.`,children:(0,a.jsx)(n,{sections:i})})})}export{o as component};